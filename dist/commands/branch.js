...
async function branch() {
    ...
    const currentHeadName = currentHeadPathElements[currentHeadPathElements.length - 1];
    // Print branch output
    printBranches(uniqueBranches, currentHeadName);
}
function printBranches(branches, currentBranch) {
    console.log("\n");
    ...
    for (const branch of branches) {
        if (branch === currentBranch)
            ...
            console.log(chalk.white(`${branch}`));
    }
    console.log("----------------------------------------------------------");
    console.log("\n");
}
